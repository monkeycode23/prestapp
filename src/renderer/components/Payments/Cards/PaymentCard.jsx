...
const PaymentCard = ({ payment }) => {
  ...
  useEffect(() => {
    const checkNotes = async () => {
      ...
      setHasNotes(notes && notes.length > 0);
    };
    checkNotes();
  }, [payment.id]);
  ...
  const pay = async (e) => {
    ...
    console.log("Asdaaaaaaaaaaaasd asdasduasdasd   payameasdjask",
          ...
          <div className="flex items-center gap-2">
            {hasNotes && (
              <div className={`rounded-full p-1 flex items-center justify-center ${
                payment.status === "incomplete" ? "bg-yellow-600" : "bg-yellow-400"
              }`}>
        ...
        <button
          ...
          onClick={async () => {
            ...
            await window.database.models.Notes.createNote({
              notes: `Monto abonado (recargo): $${formatAmount(interest)}`,
              payment_id: payment.id
            });

            dispatch(setBruteGains(bruteGains + interest));
            dispatch(setNetGains(netGains + interest));
          }}
          className={`flex w-full items-center gap-2 ${
            payment.status === "paid" ? "opacity-30 cursor-not-allowed" : ""
          } text-black rounded-sm px-4 py-1.5 text-left text-sm hover:bg-gray dark:hover:bg-meta-4`}
